...
// --- UI要素取得・生成の共通関数 ---
function getOrCreateElem<T extends HTMLElement>(id: string, tag: string): T {
...
function handlePlayerInfo(info: { id: string; name: string }) {
  ...
  getOrCreateElem<HTMLParagraphElement>('server-message', 'p').textContent = `あなたは「${myPlayerName}」です`;
}

function handlePlayerList(players: { id: string; name: string }[]) {
  console.log('受信: playerList', players);
  getOrCreateElem<HTMLParagraphElement>('player-list', 'p').textContent = '参加中プレイヤー: ' + players.map(p => p.name).join(', ');
}

function handleDeckInfo(info: { remaining: number }) {
  deckRemaining = info.remaining;
  getOrCreateElem<HTMLParagraphElement>('deck-info', 'p').textContent = `山札の残り枚数: ${info.remaining}`;
}

function handleHandInfo(hands: { [id: string]: number }) {
  const myHand = hands[myPlayerId] || 0;
  getOrCreateElem<HTMLParagraphElement>('hand-info', 'p').textContent = `あなたの手札: ${myHand}枚`;

  // 全員分の手札枚数表示
  const playerListElem = document.getElementById('player-list');
  const allHandsElem = getOrCreateElem<HTMLParagraphElement>('all-hands-info', 'p');
  if (playerListElem) {
...
function handleFieldCard(card: { suit: string; value: number } | null) {
  ...
  lastFieldCard = card;
  const fieldElem = getOrCreateElem<HTMLDivElement>('field-card', 'div');
...
function setupUI() {
  // ゲーム開始ボタン
  const startBtn = getOrCreateElem<HTMLButtonElement>('start-game-btn', 'button');
  ...
  // 参加中プレイヤー欄の初期化
  const playerListElem = getOrCreateElem<HTMLParagraphElement>('player-list', 'p');
